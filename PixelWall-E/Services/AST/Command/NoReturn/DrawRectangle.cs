    ...
    {   
        ...
        try{
            ...
            for (int x = startX; x <= endX; x++)
            {
                ...
                await PipeLineManager.Draw(x, startY, PipeLineManager.brushColor);
                Console.WriteLine($"Drawing" + PipeLineManager.brushColor + $" at ({x}, {endY}) for rectangle");
                await PipeLineManager.Draw(x, endY, PipeLineManager.brushColor);
            }
            ...
            for (int y = startY + 1; y < endY; y++) // Evitar redibujar las esquinas
            {
                ...
                await PipeLineManager.Draw(startX, y, PipeLineManager.brushColor);
                Console.WriteLine($"Drawing" + PipeLineManager.brushColor + $" at ({endX}, {y}) for rectangle");
                await PipeLineManager.Draw(endX, y, PipeLineManager.brushColor);
            }
